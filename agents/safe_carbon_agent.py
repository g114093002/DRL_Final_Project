...
class SafeCarbonAwareAgent(BaseAgent):
    def __init__(self, state_dim=12, action_dim=2):
        ...
        self.weights_path = "ppo_actor.pt"
        self.has_pretrained = False
        if os.path.exists(self.weights_path):
            try:
                # Load with map_location to ensure CPU compatibility on Streamlit Cloud
                self.actor.load_state_dict(torch.load(self.weights_path, map_location=torch.device('cpu')))
                self.actor.eval()
                self.has_pretrained = True
                print("Successfully loaded pre-trained PPO weights from ppo_actor.pt")
            except Exception as e:
                print(f"Error loading pre-trained PPO weights: {e}")
        
    def select_action(self, obs):
        if self.has_pretrained:
            # --- Actual Neural Network Decision ---
            state_t = torch.FloatTensor(obs).unsqueeze(0)
            with torch.no_grad():
                mu, _ = self.actor(state_t)
                action_val = np.clip(mu.item(), -1.0, 1.0)
            return np.array([action_val, 0.0])
        else:
            # --- High-Performance Heuristic Fallback ---
